...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # This only occurs if a ghost has been captured
        if(noisyDistance is None):

            # Values are set back to 0, except for captured ghost location
            for index in self.beliefs:
                allPossible[index] = 0

            # Captured the ghost, now mark it captured
            allPossible[self.getJailPosition()] = 1

        # Normal move, no captured ghosts
        else:
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    # Update the actual with the emission model distance
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        for currentPosition in self.legalPositions:

            # Determine the next location for the ghost
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, currentPosition))

            # print newPosDist
            # Get the next coordinates and the probabilty from the location info
            for newPos, prob in newPosDist.items():

                # print "coordinates : " + str(newPos)
                # print "moveProb : " + str(prob)

                # Modify the values within the beliefs container by the movement prob
                allPossible[newPos] += prob * self.beliefs[currentPosition]

        # Normalization taken from the observe function
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # Initialize the number of particles counter, index and number of legal positions
        particleCount = self.numParticles
        legalPositionCount = len(self.legalPositions)
        index = 0

        # While there are still particles, uniformly add them to the list. The list is
        #   initially set to the empty list in the constructor
        while particleCount != 0:
            self.particleCoordinates.append(self.legalPositions[index])

            # Increment the index and check to see it does not go over the number of
            #   legal positions
            index += 1
            index = index % legalPositionCount

            # Decrement the number of particles to add
            particleCount -= 1

        #print self.particleCoordinates
    ...
    def observe(self, observation, gameState):
        ...
        # If the ghost has been captured, then set all the particles to the jail position
        if noisyDistance is None:
            tempList = []
            for i in range(self.numParticles):
                tempList.append(self.getJailPosition())
            
            self.particleCoordinates = tempList

        else:

            # Otherwise, find the new distances using manhattan distance
            newDists = util.Counter()
            for coordinate in self.particleCoordinates:
                newDists[coordinate] += emissionModel[util.manhattanDistance(pacmanPosition, coordinate)]
            newDists.normalize()

            # If all particles have weight of 0
            if(newDists.totalCount() is 0):
                self.initializeUniformly(gameState)

            # Otherwise save the new coordinates
            else:
                tempList = []

                # Use the sample utility to generate the new particles based on the
                #   manhattan distances as the distribution
                for i in range(self.numParticles):
                    tempList.append(util.sample(newDists))
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
        # Sample the position distribution using the particle coordinates
        newCoordniates = []
        for oldPos in self.particleCoordinates:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            newCoordniates.append(util.sample(newPosDist))

        # Set the new coordinates
        self.particleCoordinates = newCoordniates
    ...
    def getBeliefDistribution(self):
        ...
        # Iterate through all of the particles and increment the distribution to
        #   entice Pacman to come closer
        for particle in self.particleCoordinates:
            beliefDistribution[particle] += 5.0
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Generate the coordinates using itertools
        self.particleCoordinates = list(itertools.product(self.legalPositions, self.legalPositions))
    ...
    def observeState(self, gameState):
        ...
        # Adjust the particles to represent the manhattan distances
        newCoordinates = util.Counter()
        for coordinates in self.particleCoordinates:
            # Initialize the weight to be 1.0, but will be modified by each of the ghosts
            weight = 1.0

            # Go through each ghost and see how it affects the current particle
            for ghost in range(self.numGhosts):

                # We captured the ghost
                if (noisyDistances[ghost] is None):
                    coordinates = self.getParticleWithGhostInJail(coordinates, ghost)

                # If we have not captured the ghost
                else:
                    # emissionIndex = util.manhattanDistance()
                    # Get the location of the ghost
                    ghostLoc = coordinates[ghost]

                    # Now update the weight to be based on the distance to the ghost
                    #   using the distribution
                    weight *= emissionModels[ghost][util.manhattanDistance(ghostLoc, pacmanPosition)]
                    #print "Base weight " + str(weight)

            # Now modify the weight of the given particle by the new weight
            newCoordinates[coordinates] += weight

        # Normalize all of the respective particles
        newCoordinates.normalize()

        # If we have coordinates that exist (weights after normalization == 1)
        if newCoordinates.totalCount():

            # Now re-sample all of the coordinates and reset the coordinates
            tempList = []

            for i in range(self.numParticles):
                tempList.append(util.sample(newCoordinates))

            self.particleCoordinates = tempList

        # There are no particles to use, so re-initialize them
        else:
            # Re-initialize the particles
            self.initializeParticles()

            # Put the ghosts that were eaten into jail
            for ghost in range(self.numGhosts):

                # Ghost has been eaten
                if noisyDistances[ghost] is None:
                    self.particleCoordinates[ghost] = self.getParticleWithGhostInJail(
                        self.particleCoordinates[ghost], ghost)
    ...
    def elapseTime(self, gameState):
        ...
        """
        newParticles = []
        for oldParticle in self.particleCoordinates:
            ...
            for i in range(self.numGhosts):
                ...
                # Now sample the new particle location using the new distribution
                newParticle[i] = util.sample(newPosDist)

            "*** END YOUR CODE HERE ***"
            newParticles.append(tuple(newParticle))
        self.particleCoordinates = newParticles
