...
:- checkgoals([is(A,+(3,2)),is(_,+(4,5)),C is max(5,2),A=C]).  %fails
:- checkgoals([=(A,5),is(B,+(1,2)),B is max(9,A)]) -> fail ; true. %fails
...
/* Problem 2 Answer: */
computeS(Op,Arg1,Arg2,Result) :- Temp =.. [Op,Arg1,Arg2], Result is Temp.
...
/* Problem 4 Answer */
sumlist([], 0).
sumlist([H|T], R) :- sumlist(T,Temp), R is Temp + H.
...
sumlist2([], Sum, Sum).
sumlist2([H|T], Temp, Sum) :- Tp is +(H,Temp), sumlist2(T,Tp,Sum).
...
/* Problem 6 Answer */
sumPartialR(1,[1]).
sumPartialR(N, [H1,H2|T]) :- Nxt is N-1, sumPartialR(Nxt,[H2|T]), H1 is N+H2.
...
/* Problem 7 Answer */
sumPartialL(N,Lst):-sumPartialL(N,N,Lst).
sumPartialL(1,Sum,[Sum]).
sumPartialL(N,Sum,[H1,H2|T]) :- Next is N-1, NewNext is Next + H1, H1 = Sum,
                                sumPartialL(Next, NewNext, [H2|T]).
...
/* Problem 8 Answer */
outgoing(X,Y) :- findall(N, edge(X, N), Y).
incoming(X,Y) :- findall(N, edge(N, X), Y).
...
/* Problem 10 Answer: */
swap(leaf(X), leaf(X)).
swap( tree(X,Y), tree(X2,Y2) ) :- swap(X,Y2), swap(Y,X2).
...
/* Problem 11 Answer: */
ack(0, N, A) :- A is N+1.
ack(M, 0, A) :- NewM is M-1, ack(NewM, 1, A).
ack(M, N, A) :- NewM is M-1, NewN is N-1, ack(NewM, ack(M, NewN, A), A).
