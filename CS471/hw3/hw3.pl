...
source file. BEFORE MOVING ON RENAME hw3S19.asn to hw3.pl. You will be
...
Each line is an individual Prolog query; it's a good idea type them in your prompt
(not the file itself) to get a feel for the way Prolog works. You should think
about whether or not each query will succeed, and if so how the variables will
be instantiated (as result of unification). You can expect these sort of
...
/* Problem 1 Answer: */
    /* Answer:
        A)
            a) A relation between two sets is a collection of ordered pairs containing
                one object from each set.
            b) A set of finite lists of objects, one for each combination of possible
                arguments.
        B)
            Every function is a relation.
        C)
            Not every relation is a function. A counterexample would be:
                A function cannot possibly have more than one value for one combination
                of arguments. However, a relation can. If we define a function called f(x),
                which takes a number in, and returns x+1, then you cannot get x+2 or x-1 or
                just x as an answer. But if you defined a relation x > 2, you will get
                multiple values for x (3,4,5...).
        */
...
/* Problem 2 Answer: */
    /* Answer:
        Homoiconic means that it is a program that "look[s] like ordinary data structures,
        and can be created, modified, and executed on the fly." (Kaufmann 584).

        Prolog is a homoiconic language since it can be modified on the fly, meaning that
        the written Prolog code can be changed by other Prolog code.

        A language is fully reflexive if it can modify its own structure and behavior at
        runtime.

        Prolog is also fully reflexive because its code can be changed at runtime. You can
        add rules and laws while a Prolog program executes.
    */
/* Problem 3:
...
/* Problem 4 Answer: */
init([_],[]).
init([X1|Y1], [X2|Y2]) :- X1 == Y2, init(X1,Y2).
...
/* Problem 5:
Write a predicate is_decreasing(X) that succeeds if X is a list of decreasing numbers
-- Each number is either the same or lower than the preceding number.

NOTE: You may match two elements at a time against a list: [X,Y|Xs] = List.
It's preferable to do it in the rule head however...some_rule([X,Y|Xs]) :- ...  */

/* Problem 5 Answer: */
is_decreasing([]).
is_decreasing([_]).
is_decreasing([X1,X2|Y]) :- X1>=X2, is_decreasing([X2|Y]).
...
/* Problem 6:
Write a predicate element_at(X,Y,N) that succeeds if X is the Nth element of list Y.
Y is 0-indexed.
...
/* Problem 7 :
Write a predicate insert_at(E,Y,N,Z) that succeeds if Z is the list Y with E inserted
at index N -- Insert X at index N in Y.

NOTE: Don't worry about the error cases: i.e, N greater than the length of Y.  */

/* Problem 7 Answer: */
insert_at(X,[],0,[X]).
insert_at(X,[H|T],0,[X,H|T]).
insert_at(X, [_|T], N, [_|T2]) :- M is N-1, insert_at(X,T,M,T2).
...
/* Problem 8 :
Write a predicate delete_at(E,Y,N,Z) that succeeds if Z is the list Y with E deleted
at index N -- Delete E at index N in Y.
...
/* Problem 8 Answer: */
% Below is the solution without using insert_at(E,Y,N,Z).
%delete_at(X,[X],0,[]).
%delete_at(X1,[X1,X2|T],0,[X2|T]).
%delete_at(X, [_|T], N, [_|T2]) :- M is N-1, delete_at(X,T,M,T2).

% Below is the solution using insert_at(E,Y,N,Z).
delete_at(X, [_|T], N, [_|T2]) :- insert_at(X,[_|T2],N,[_|T]).
...
/* Problem 9:
Write a predicate zip(Xs,Ys,Zs) that succeeds if Zs is a list where each element is
a tuple, (X,Y), with Xs and Ys paired together.
...
Write a predicate zip2(Xs,Ys,Zs) that succeeds if Zs is a list where each element is
a tuple, (X,Y), with Xs and Ys paired together. However, the length of Zs will be equal to the length of Xs or Ys which ever is less.
...
zip2([],_,[]).
zip2([],[],[]).
zip2([X|T1],[Y|T2], [(X,Y)|T3]) :- zip2(T1,T2,T3).
...
/* Problem 11:
Write a predicate merge(A,B,M) that succeeds if the list M has all the items from lists
A and B in decreasing order.  Assume that A and B are sorted in decreasing order.
Items do not need to be unique.
...
/* Problem 11 Answer: */
check_less(X,Y,[X,Y]) :- X>=Y.
check_less(X,Y,[Y,X]) :- Y>=X.
my_append([],X,X).
my_append([H|T],X,[H|T2]) :- my_append(T,X,T2).
merge([],X,X).
merge(X,[],X).
merge([X|T1], [Y|T2], M) :- check_less(X,Y,Middle), merge(T1,T2,Final), my_append(Middle,Final,M).
...
Define subtract(Num1,Num2,Result) to succeed if Result is the result of
...
/* Problem 13 Answer: */
subtract(X,0,X).
subtract(succ(X),succ(Y), R) :- subtract(X,Y,R).
...
Write a predicate has_subseq(X,Y) that succeeds if Y is a list that is a subsequence
of a list X.

For example...
has_subseq([a,b,c,d],[b,d]) should succeed, but has_subseq([a,b,c,d],[b,e]) should
fail. */

/* Problem 14 Answer */
has_subseq([],[]).
has_subseq(List,[]).
has_subseq([X|T1],[Y|T2]) :- X==Y, has_subseq(T1,T2).
has_subseq([A|T3],[B|T4]) :- A\==B, has_subseq(T3,[B|T4]).
...
/* Problem 15:
(From Learn Prolog NOW!) Binary trees are trees where all internal nodes have exactly
two children. The smallest binary trees consist of only one leaf node. We will
represent leaf nodes as leaf(Label). For instance, leaf(3) and leaf(7) are leaf
nodes, and therefore small binary trees. Given two binary trees B1 and B2 we can
combine them into one binary tree using the predicate tree: tree(B1,B2). So, from
the leaves leaf(1) and leaf(2) we can build the binary tree: tree(leaf(1), leaf(2)).
And from the binary trees: tree(leaf(1), leaf(2)) and leaf(4) we can build the binary
tree: tree(tree(leaf(1), leaf(2)), leaf(4)).

Now define a predicate isBinaryTree(+BT) which succeeds if BT is a binary tree. The "+"
indicates that it is assumed BT is instantiate in the query.
