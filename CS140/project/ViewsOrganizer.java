...
public class ViewsOrganizer extends Observable {
	...
	private void createAndShowGUI(){
		...
		memoryViewPanel3 = new MemoryViewPanel(this, model, Memory.DATA_SIZE/2,
																	Memory.DATA_SIZE);
		...
		model.setCurrentStates(States.NOTHING_LOADED);
		animator.start();
	...
	public void toggleAutoStep(){
		...
		if(animator.isAutoStepOn()){
			model.setCurrentStates(States.AUTO_STEPPING);
		}else{
			model.setCurrentStates(States.PROGRAM_LOADED_NOT_AUTOSTEPPING);
		}
		model.getCurrentState().enter();
		setChanged();
		notifyObservers();
